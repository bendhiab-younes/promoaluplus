...
| 🟡 | 3 | The same CTA button block is hand-rolled 4 times with raw utility classes instead of the shared `.btn-primary`/`.btn-secondary` classes, and all 4 copies use `mr-2` (not `me-2`) on the icon — duplicated markup *and* an RTL bug baked in 4 places. **(RTL `me-2` fixed; consolidation into a shared partial not done.)** | `home.blade.php:642-648`, `services.blade.php:296-301`, `portfolio.blade.php:213-218`, `about.blade.php:305-310` | High |
...
| ⏳ | 17 | The same decorative SVG dot-pattern background + inline `<style>` is redeclared independently across pages instead of one shared layout-level class. | multiple | Low |
...
| 🟡 | 9 | No `<meta name="robots">`; no dedup for `?category=` variants. **(Added `<meta name="robots" content="index, follow, max-image-preview:large">`; the `?category=` thin-duplicate dedup is NOT solved — canonical is self-referential incl. query string. See note below.)** | `layouts/app.blade.php` | Medium |
...
| ⏳ | 11 | Portfolio project `alt` text is just the title. **(Deferred — Low.)** | `portfolio.blade.php:47` | Low |
| ⏳ | 12 | Logo `alt` is brand-only. **(Deferred — Low.)** | `home.blade.php:749`, `layouts/app.blade.php:825` | Low |
...
> **Note on SEO #9 (`?category=` dedup):** the canonical tag now emits `url()->current()`, which *includes* the query string, so `/portfolio?category=doors` self-canonicalizes rather than pointing back to `/portfolio`. If crawl-budget/thin-duplicate becomes a concern, either emit a canonical that strips the query string on the portfolio page, or add `<meta name="robots" content="noindex,follow">` for filtered variants.
...
6. **Low-severity polish (UI #17, Perf #12, SEO #11, #12)** — deferred as marginal; easy to pick up later.
